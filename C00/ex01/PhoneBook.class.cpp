...
void PhoneBook::Add_Contact(void){
    ...
    std::cout << "Contact " << contacts[index].get_string("first_name") << " added!" << std::endl;
...
void PhoneBook::Search_Contact(void){
    ...
    while (!contacts[0].get_string("first_name").empty()){
        Input = clean_input("Please, enter the index of the contact you are looking for:");
        if (Input >= "0" && Input <= "7"){
            ...
            if (to_find >= 0 && to_find < i){
                ...
                std::cout << "Darkest Secret : " << contacts[to_find].get_string("darkest_secret") << std::endl;
                break;
            }
            else
